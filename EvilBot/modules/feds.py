...
import EvilBot.modules.sql.feds_sql as sql
from EvilBot import (
    EVENT_LOGS,
    LOGGER,
    SUPPORT_CHAT,
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "evil_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="evil_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "evil_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="evil_fbanned_users.csv",
